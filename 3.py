...
def cities_game():
    # Инициализируем сессию для игры в города, если её ещё нет
...
def elephant():
    # Инициализируем сессию для игры «Купи слона», если её ещё нет
    ...
    if form.validate_on_submit():
        ...
        session['elephant_message_list'].append((last_elephant_message, 0))
        if last_elephant_message.lower() in end_game_message:
            ...
            session['elephant_message_list'].append((f'Все говорят "{last_elephant_message}", а ты купи слона!', 1))
        form.answer.data = ''
